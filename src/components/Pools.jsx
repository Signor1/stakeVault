...
const Pools = () => {
    ...
    return (
        <MaxWidth className={`w-full md:px-6 mt-10`}>
            <section className="w-full flex flex-col md:p-8 p-6 bg-slate-900 rounded-md">
                <div className="w-full flex justify-between items-center border-b border-slate-500/50 pb-6">
                    <h1 className="text-slate-100 font-serif font-bold md:text-lg text-sm">List of Staking Pools</h1>

                    <div className="flex gap-2">
                        {isConnected && (<button onClick={handleApproval} className="bg-blue-500 flex items-center gap-1 text-sm px-6 py-1.5 rounded text-slate-100">Approve
                            <MdArrowRightAlt className="text-xl" />
                        </button>)}

                        <Dialog.Root>
                            <Dialog.Trigger>
                                {isConnected && (<button className="bg-blue-500 flex items-center gap-1 text-sm px-6 py-1.5 rounded text-slate-100">Add Pool
                                    <MdArrowRightAlt className="text-xl" />
                                </button>)}
